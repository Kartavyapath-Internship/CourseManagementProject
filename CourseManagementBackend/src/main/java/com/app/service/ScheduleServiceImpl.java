...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public ScheduleRespDto addSchedule(ScheduleReqDto dto) {
        ...
        List<Infrastructure> infraList = dto.getInfrastructureId().stream()
        		.map(infra -> infrastructureDao.findById(infra).orElseThrow(() -> new RuntimeException("Infrastructure not found by id "+infra)))
        		.toList();
        
        schedule.setInfrastructures(infraList);
        
        List<Group> groupList = dto.getGroupIds().stream()
        		.map(group -> groupDao.findById(group).orElseThrow(() -> new RuntimeException("Group not found by id "+group)))
        		.toList();
        
        schedule.setGroups(groupList);

        List<Staff> staffList = dto.getStaffId().stream()
                .map(staff -> staffDao.findById(staff).orElseThrow(() -> new RuntimeException("staff not found by id "+staff)))
                .toList();
    ...
    public ScheduleRespDto updateSchedule(int id, ScheduleReqDto dto) {
        ...
        Schedule schedule = scheduleDao.findById(id).orElseThrow(() -> new RuntimeException("Schedule not found by id "+id));
